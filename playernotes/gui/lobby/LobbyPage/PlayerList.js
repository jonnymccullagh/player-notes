...
class PlayerList
{
	constructor(xmppMessages, buddyButton, gameList)
	{
		...
		this.playersBox.onHoverChange = this.onHoverChange.bind(this);
	...
	selectPlayer()
	{
		const index = this.playersBox.list_data.indexOf(this.playersFilter.caption);
		if (index != -1)
	...
	onHoverChange()
	{
		if (this.playersBox.hovered === -1)
		{
			this.playersBox.tooltip = "";
			return;
		}

		this.playerNoteStore.reload();
		this.playersBox.tooltip = this.playerNoteStore.formatTooltip(
			this.playersBox.list_data[this.playersBox.hovered]);
	}

	onPlayerListSelection()
	{
		if (this.playersBox.selected == this.playersBox.list_data.indexOf(this.selectedPlayer))
			return;

		this.selectedPlayer = this.playersBox.list_data[this.playersBox.selected];
	...
	rebuildPlayerList()
	{
		...
		this.rawNickList.length = length;
		...
		playerList.forEach((player, i) =>
		{
			...
			const hasNotes = !!this.playerNoteStore.getNotes(player.name).length;
			const noteMarker = hasNotes ? setStringTags(this.NoteMarker, this.NoteMarkerTags) : "";
			this.buddyStatusList[i] = player.isBuddy ? setStringTags(g_BuddySymbol, statusTags) : "";
			this.playerList[i] = noteMarker + PlayerColor.ColorPlayerName(player.name, "", player.role);
			...
			this.rawNickList[i] = player.name;
		});
		Engine.ProfileStop();
		...
		this.playersBox.list_data = this.rawNickList;
		Engine.ProfileStop();

		Engine.ProfileStart("selectionChange");
		this.playersBox.selected = this.playersBox.list_data.indexOf(this.selectedPlayer);
		Engine.ProfileStop();
...
PlayerList.prototype.NoteMarker = g_NotesSymbol + " ";

PlayerList.prototype.NoteMarkerTags = {
	"color": "237 227 167"
};
